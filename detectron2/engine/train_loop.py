...
class SimpleTrainer(TrainerBase):
    ...
    def run_step(self):
        ...
        if self.cfg.FAKE.FLAG:
            dir=self.cfg.FAKE.LOAD_DIR
            for d in data:
                image_id=d['image_id']
                xml_path=os.path.join(dir,image_id+'.xml')
                if not os.path.exists(xml_path):continue
                with PathManager.open(xml_path) as f:
                    tree = ET.parse(f)
                    boxes=[];classes=[]
                    for obj in tree.findall("object"):
                        ...
                        boxes.append(box)
                    fake_box=Boxes(torch.Tensor(boxes))#,dtype=torch.float32
                    fake_cls=torch.Tensor(classes)#,dtype=torch.int64
                    d['instances']._fields['gt_boxes']=Boxes.cat([d['instances']._fields['gt_boxes'],fake_box])
                    d['instances']._fields['gt_classes']=torch.cat([d['instances']._fields['gt_classes'],fake_cls],dim=0).to(torch.int64)
