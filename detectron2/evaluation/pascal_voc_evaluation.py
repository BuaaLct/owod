...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg=None):
        ...
        if cfg is not None:
            ...
            if cfg.FAKE.FLAG:
                self.fake_flag=True
                self.fake_save_dir=cfg.FAKE.SAVE_DIR
                self.blank_xml_path=cfg.FAKE.BLANK_DIR
                self.fake_thresh=cfg.FAKE.THRESH
            else:
    ...
    def insert_object(self,box, name, difficult, root):
        ...
        difficult_node = Element('difficult')
        name_node.text = name
        difficult_node.text = difficult
        one1.append(two1)
        one1.append(two2)
        one1.append(two3)
        one1.append(two4)
        element.append(name_node)
        element.append(difficult_node)
        element.append(one1)
        root.append(element)
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            if self.fake_flag:
                ...
                base_tree=ET.parse(self.blank_xml_path)
            ...
            for box, score, cls in zip(boxes, scores, classes):
                ...
                if self.fake_flag and score > self.fake_thresh:
                    fake_instance_count+=1
                    self.insert_object([xmin, ymin, xmax, ymax], str(cls) , "0" , base_root)

            if self.fake_flag and fake_instance_count>0:
                base_tree.write(os.path.join(self.fake_save_dir,image_id+'.xml'), encoding='utf-8')
